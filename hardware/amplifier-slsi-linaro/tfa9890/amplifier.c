 ...
 *
 * @param snd_device The current sound device (device_type_t or SND_DEVICE value).
 *
...
static tfa_mode_t classify_snd_device(uint32_t snd_device) {
    ...
    // Handle device_type_t values from audio-hal_slsi-linaro
    // DEVICE_EARPIECE=0, DEVICE_SPEAKER=1, DEVICE_HEADSET=2, DEVICE_HEADPHONE=3,
    // DEVICE_SPEAKER_AND_HEADSET=4, DEVICE_SPEAKER_AND_HEADPHONE=5
    switch (snd_device) {
        case 1:  // DEVICE_SPEAKER
        case 4:  // DEVICE_SPEAKER_AND_HEADSET
        case 5:  // DEVICE_SPEAKER_AND_HEADPHONE
        ...
        uint32_t devices, bool enable)
{
    ...
    int rc = 0;
    int total_speaker_refcount = 0;

    ALOGV("%s: devices=0x%x, enable=%d, tfa_mode=%d", __func__, devices, enable, tfa_mode);

    // For non-speaker devices, just return without doing anything
    ...
    // Calculate total speaker refcount across all speaker modes
    total_speaker_refcount = dev->refcount[Audio_Mode_Music_Normal];

    ALOGV("%s: enable=%d, dev->refcount=%d, total_speaker_refcount=%d, amp_state=%d",
          __func__, enable, dev->refcount[tfa_mode], total_speaker_refcount, dev->amp_state);

    if (enable && dev->amp_state == AMP_STATE_DISABLED && total_speaker_refcount == 1) {
        rc = tfa_power(dev->tfa_dev, true);
        if (rc == 0) {
            dev->amp_state = AMP_STATE_ENABLED;
        }
        ALOGV("%s: tfa_power(true) with rc=%d", __func__, rc);
    } else if (!enable && dev->amp_state == AMP_STATE_ENABLED && total_speaker_refcount == 0) {
...
static int amp_output_stream_start(amplifier_device_t *device __attribute__((unused)),
        struct audio_stream_out *stream, bool offload)
{
    ALOGV("%s: stream=%p, offload=%d", __func__, stream, offload);
    /* TFA doesn't need stream lifecycle notifications */
    return 0;
}

static int amp_input_stream_start(amplifier_device_t *device __attribute__((unused)),
...
static int amp_output_stream_standby(amplifier_device_t *device __attribute__((unused)),
...
static int amp_input_stream_standby(amplifier_device_t *device __attribute__((unused)),
        struct audio_stream_in *stream)
{
    ALOGV("%s: stream=%p", __func__, stream);
    /* TFA doesn't need stream lifecycle notifications */
        ...
        hw_device_t **device)
{
    ...
    amp_dev->amp_dev.input_stream_start = amp_input_stream_start;
    amp_dev->amp_dev.output_stream_standby = amp_output_stream_standby;
    amp_dev->amp_dev.input_stream_standby = amp_input_stream_standby;
...
amplifier_module_t HAL_MODULE_INFO_SYM = {
    .common = {
        ...
        .name = "Samsung TFA9890 amplifier HAL",
